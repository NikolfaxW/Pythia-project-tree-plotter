...
void
DrawHistogramFromTree(const std::string path, const std::string filename, const char *treename, const char *variable,
...
//    legend->SetMargin(1);
    ...
    legend->AddEntry(histogram, "1.2 * 10^{6} events", "");
...
void DrawHistogramFromTreeTest(std::string path_to_tree, std::string output_file_name, std::string tree_name,
    ...
    for(int i = 0; i < size; ++i){
        ...
        histogram[i] = new TH1F(temp.c_str(), title.c_str(), 100, left_border, right_border);
    }
